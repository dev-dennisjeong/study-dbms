...
SELECT TEAM_ID, PLAYER_ID, PLAYER_NAME, WEIGHT 
FROM PLAYER
...
WHERE HEIGHT >= 170
AND WEIGHT >= 80;
/*STADIUM 테이블에서 SEAT_COUNT가 30000초과이고 41000이하인 경기장*/
SELECT STADIUM_ID, SEAT_COUNT 
FROM STADIUM
...
SELECT TEAM_ID, PLAYER_ID, PLAYER_NAME  FROM PLAYER;
...
/*선수 이름과 생일 조회*/
SELECT PLAYER_NAME "선수 이름", BIRTH_DATE "생일"  FROM PLAYER;
...
/*PLAYER_NAME의 영어이름은 E_PALYER_NAME이다.*/
SELECT PLAYER_NAME || '의 영어이름은' || E_PLAYER_NAME  || '이다' AS 영어이름 FROM PLAYER;
/*PLAYER_NAME의 포지션은 PISITION입니다.*/
...
SELECT TEAM_NAME  FROM TEAM WHERE TEAM_NAME LIKE '%천마';
/*PLAYER 테이블에서 김씨 찾기*/
...
OR PLAYER_NAME LIKE '이%';
/*PLAYER 테이블에서 이씨가 아닌 사람 찾기(NOT)*/
/*ANSI 표준 문법*/
SELECT PLAYER_NAME  FROM PLAYER WHERE NOT PLAYER_NAME LIKE '이%';
/*ORACLE 문법*/
SELECT PLAYER_NAME  FROM PLAYER WHERE PLAYER_NAME NOT LIKE '이%';
...
SELECT COUNT(NVL(HEIGHT, 0)) "총 선수 명수" FROM PLAYER;
...
SELECT "POSITION", AVG(HEIGHT) 평균키,  MIN(WEIGHT) "최소 몸무게" FROM PLAYER
WHERE WEIGHT >= 80
HAVING AVG(HEIGHT)>=180
GROUP BY "POSITION";

/*EMPLOYEES 테이블에서 JOB_ID별 평균 SALARY가 10000미만인 JOB_ID 검색, JOB_ID 알파벳순 정렬*/
SELECT JOB_ID, AVG(SALARY) "평균 급여" FROM EMPLOYEES
GROUP BY JOB_ID
...
SELECT "POSITION", AVG(HEIGHT) "포지션 별 평균키", (SELECT AVG(HEIGHT) FROM PLAYER) "전체 평균 키" 
FROM PLAYER
WHERE "POSITION" IS NOT NULL
GROUP BY "POSITION";
...
SELECT NICKNAME FROM PLAYER WHERE NICKNAME = '킹카';
...
ON E.HIRE_DATE BETWEEN '2003-01-01' AND '2005-12-31';
...
--부서ID 중에 사원들의 이름과 사원들 중 매니저인 사원의 이름을 조회
...
FROM DEPARTMENTS D JOIN EMPLOYEES E
ON E.EMPLOYEE_ ID = D.MANAGER_ID;


SELECT ET.DEPARTMENT_ID, 
	DT.FIRST_NAME || ' ' || DT.LAST_NAME AS "사원 이름",
	ET.FIRST_NAME || ' ' || ET.LAST_NAME AS "매니저 이름"
FROM
(
	SELECT E.DEPARTMENT_ID, E.FIRST_NAME, E.LAST_NAME
	FROM EMPLOYEES E
